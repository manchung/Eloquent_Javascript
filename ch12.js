function parseExpression(program) {
  ...
  function skipSpace(string) {
    ...
    if (string[first] == '#') {
        ...
        if (newline == -1) return "";
        // console.log(`newline: ${newline}  string slice: ${string.slice(newline)}`);
        return skipSpace(string.slice(newline));
    } else {
  ...
  specialForms.array = (args, scope) => {
    return args.map(a => evaluate(a, scope))
  }
  ...
  specialForms.element = (args, scope) => {
    if (args.length != 2) { 
        ...
        let n = evaluate(args[1], scope);
        return array[n];
    }
  }

run(`do( define(a, array(1, 2)), 
    # this is a comment
    # another comment
    print(element(a, 0))   )  `)

let ss = `do( define(a, array(1, 2)), 
    # this is a comment
    # this is another comment
    define(n, 0),
    while(<(n, 2), do(print(element(a, n)), ))
    print(element(a, 0))   )  `
